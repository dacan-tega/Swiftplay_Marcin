...
if ($formsOnly) {
    safeUnlink(__DIR__ . '/src/SlotgenLucky81Theme.php');
    safeUnlink(__DIR__ . '/src/SlotgenLucky81Plugin.php');
    ...
    if ($isTheme) {
        safeUnlink(__DIR__ . '/src/SlotgenLucky81ServiceProvider.php');
        safeUnlink(__DIR__ . '/src/SlotgenLucky81Plugin.php');
        safeUnlink(__DIR__ . '/src/SlotgenLucky81.php');
...
foreach ($files as $file) {
    replaceInFile($file, [
        ...
        'slotgen-lucky81' => $packageSlug,
        'slotgen-lucky_81_without_prefix' => $packageSlugWithoutPrefix,
        'SlotgenLucky81' => $className,
        'slotgen-lucky81' => $packageSlug,
        'migration_table_name' => titleSnake($packageSlug),
        'variable' => $variableName,
        'Lucky 81 plugin for Filament' => $description,
    ]);

    match (true) {
        str_contains($file, determineSeparator('src/SlotgenLucky81.php')) => rename($file, determineSeparator('./src/' . $className . '.php')),
        str_contains($file, determineSeparator('src/SlotgenLucky81ServiceProvider.php')) => rename($file, determineSeparator('./src/' . $className . 'ServiceProvider.php')),
        str_contains($file, determineSeparator('src/SlotgenLucky81Theme.php')) => rename($file, determineSeparator('./src/' . $className . (str_ends_with($className, 'Theme') ? '.php' : 'Theme.php'))),
        str_contains($file, determineSeparator('src/SlotgenLucky81Plugin.php')) => rename($file, determineSeparator('./src/' . $className . 'Plugin.php')),
        str_contains($file, determineSeparator('src/Facades/SlotgenLucky81.php')) => rename($file, determineSeparator('./src/Facades/' . $className . '.php')),
        str_contains($file, determineSeparator('src/Commands/SlotgenLucky81Command.php')) => rename($file, determineSeparator('./src/Commands/' . $className . 'Command.php')),
        str_contains($file, determineSeparator('src/Testing/TestsSlotgenLucky81.php')) => rename($file, determineSeparator('./src/Testing/Tests' . $className . '.php')),
        str_contains($file, determineSeparator('database/migrations/create_slotgenlucky81_table.php.stub')) => rename($file, determineSeparator('./database/migrations/create_' . titleSnake($packageSlugWithoutPrefix) . '_table.php.stub')),
        str_contains($file, determineSeparator('config/slotgen-lucky81.php')) => rename($file, determineSeparator('./config/' . $packageSlugWithoutPrefix . '.php')),
        str_contains($file, determineSeparator('resources/lang/en/slotgen-lucky81.php')) => rename($file, determineSeparator('./resources/lang/en/' . $packageSlugWithoutPrefix . '.php')),
...
{
    return preg_split('/\\r\\n|\\r|\\n/', run('dir /S /B * | findstr /v /i .git\ | findstr /v /i vendor | findstr /v /i ' . basename(__FILE__) . ' | findstr /r /i /M /F:/ ":author :vendor :package Slotgen slotgen-lucky81 migration_table_name vendor_name vendor_slug author@domain.com"'));
}
...
function replaceForAllOtherOSes(): array
{
    return explode(PHP_EOL, run('find ./* ./.github/* -name "vendor" -type d -prune \
     ...
     -o -type f -print0 | xargs -0 grep -E -r -l -i ":author|:vendor|:package|Slotgen|slotgen-lucky81|migration_table_name|vendor_name|vendor_slug|author@domain.com"'));
}
