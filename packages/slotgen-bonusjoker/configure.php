...
if ($formsOnly) {
    safeUnlink(__DIR__ . '/src/SlotgenBonusJokerTheme.php');
    safeUnlink(__DIR__ . '/src/SlotgenBonusJokerPlugin.php');
    ...
    if ($isTheme) {
        safeUnlink(__DIR__ . '/src/SlotgenBonusJokerServiceProvider.php');
        safeUnlink(__DIR__ . '/src/SlotgenBonusJokerPlugin.php');
        safeUnlink(__DIR__ . '/src/SlotgenBonusJoker.php');
...
foreach ($files as $file) {
    replaceInFile($file, [
        ...
        'slotgen' => $vendorName,
        'slotgen' => $vendorSlug,
        'Slotgen' => $vendorNamespace,
        ':package_name' => $packageName,
        'slotgen-bonusjoker' => $packageSlug,
        'slotgen-bonus_joker_without_prefix' => $packageSlugWithoutPrefix,
        'SlotgenBonusJoker' => $className,
        'slotgen-bonusjoker' => $packageSlug,
        'migration_table_name' => titleSnake($packageSlug),
        'variable' => $variableName,
        'Bonus Joker plugin for Filament' => $description,
    ]);

    match (true) {
        str_contains($file, determineSeparator('src/SlotgenBonusJoker.php')) => rename($file, determineSeparator('./src/' . $className . '.php')),
        str_contains($file, determineSeparator('src/SlotgenBonusJokerServiceProvider.php')) => rename($file, determineSeparator('./src/' . $className . 'ServiceProvider.php')),
        str_contains($file, determineSeparator('src/SlotgenBonusJokerTheme.php')) => rename($file, determineSeparator('./src/' . $className . (str_ends_with($className, 'Theme') ? '.php' : 'Theme.php'))),
        str_contains($file, determineSeparator('src/SlotgenBonusJokerPlugin.php')) => rename($file, determineSeparator('./src/' . $className . 'Plugin.php')),
        str_contains($file, determineSeparator('src/Facades/SlotgenBonusJoker.php')) => rename($file, determineSeparator('./src/Facades/' . $className . '.php')),
        str_contains($file, determineSeparator('src/Commands/SlotgenBonusJokerCommand.php')) => rename($file, determineSeparator('./src/Commands/' . $className . 'Command.php')),
        str_contains($file, determineSeparator('src/Testing/TestsSlotgenBonusJoker.php')) => rename($file, determineSeparator('./src/Testing/Tests' . $className . '.php')),
        str_contains($file, determineSeparator('database/migrations/create_slotgenbonusjoker_table.php.stub')) => rename($file, determineSeparator('./database/migrations/create_' . titleSnake($packageSlugWithoutPrefix) . '_table.php.stub')),
        str_contains($file, determineSeparator('config/slotgen-bonusjoker.php')) => rename($file, determineSeparator('./config/' . $packageSlugWithoutPrefix . '.php')),
        str_contains($file, determineSeparator('resources/lang/en/slotgen-bonusjoker.php')) => rename($file, determineSeparator('./resources/lang/en/' . $packageSlugWithoutPrefix . '.php')),
...
{
    return preg_split('/\\r\\n|\\r|\\n/', run('dir /S /B * | findstr /v /i .git\ | findstr /v /i vendor | findstr /v /i ' . basename(__FILE__) . ' | findstr /r /i /M /F:/ ":author :vendor :package Slotgen slotgen-bonusjoker migration_table_name vendor_name vendor_slug author@domain.com"'));
}
...
function replaceForAllOtherOSes(): array
{
    return explode(PHP_EOL, run('find ./* ./.github/* -name "vendor" -type d -prune \
     ...
     -o -type f -print0 | xargs -0 grep -E -r -l -i ":author|:vendor|:package|Slotgen|slotgen-bonusjoker|migration_table_name|vendor_name|vendor_slug|author@domain.com"'));
}
