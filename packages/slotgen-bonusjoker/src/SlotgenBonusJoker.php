...
class SlotgenBonusJoker
{
    ...
    {
        // $api_url = route('web.bonusjoker.site.launch');
        ...
        if (File::exists($gameFile)) {
            ...
            $logo_url = url('uploads/games/' . $gameFolder . '/bonusjoker-logo.png');
        } else {
            $gameFolder = '';
    ...
    public static function LaunchGame($launchData)
    {
        ...
        $dataType = 'normal';
        SlotgenBonusJoker::spinConfig($path, $dataType);
        ...
        if ($USE_SEAMLESS) {
            $user = SlotgenBonusJoker::PlayerEntityId($userName);
        } else {
            $user = SlotgenBonusJoker::PlayerEntityId($playerId);
        }
        ...
        if (!$user) {
            ...
            if ($gameName) {
                ...
                $playerData = new BonusJokerPlayer();
                ...
                // dd($playerData->uuid);
                $user = BonusJokerPlayer::where('uuid', $playerData->uuid)->first();
                // dd($user);
                if (!$playerData) {
