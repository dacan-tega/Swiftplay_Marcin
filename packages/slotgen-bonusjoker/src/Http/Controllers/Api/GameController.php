...
namespace Slotgen\SlotgenBonusJoker\Http\Controllers\Api;
...
use Slotgen\SlotgenBonusJoker\Helpers\Common;
use Slotgen\SlotgenBonusJoker\Http\Controllers\AppBaseController;
use Slotgen\SlotgenBonusJoker\Models\BonusJokerPlayer;
use Slotgen\SlotgenBonusJoker\Models\BonusJokerSpinLogs;
use Slotgen\SlotgenBonusJoker\Models\SlotgenBonusJokerConfig;
...
class GameController extends AppBaseController
{
    public function info(Request $request)
    {
        ...
        $api_url = route('api.bonusjoker.v1.root');
        $id = isset($request['id']) ? $request['id'] : null;
        if ($act) {
            if ($act == 'payout') {
                return view('slotgen-bonusjoker::api.payout', compact('gameName', 'gamePublicFolder'));
            } elseif ($act == 'gamerule') {
                return view('slotgen-bonusjoker::api.game_rule', compact('gameName', 'gamePublicFolder'));
            } elseif ($act == 'histories') {
                return view('slotgen-bonusjoker::api.history', compact('id', 'token', 'api_url', 'gameName', 'gamePublicFolder'));
            }
        } else {
            return 'Empty action';
        }
    ...
    public function launchGame(Request $request)
    {
        ...
        if ($checkLaunchGame) {
            $player = BonusJokerPlayer::where('player_uuid', $userName)->first();
            if ($player) {
                $launchData = [
                    'uuid' => $player->uuid,
    ...
    public function history(Request $request)
    {
        ...
        return view('slotgen-bonusjoker::api.history', compact('token', 'api_url', 'gameName', 'history'));
    }

    public function GameAction(Request $request)
    {
        $adjustRatio = (object) SlotgenBonusJokerConfig::first();
        //###############
        ...
        $path = storage_path('app/private/bonus_joker');
        // $gameName = isset($p->game) ? $p->game : null;
        $gameName = 'bonus_joker';
        ...
        $sessionPlayer = BonusJokerPlayer::where('uuid', $token)->first();
        ...
        if ($sessionPlayer) {
            ...
            if ($act === 'session' || $act === 'spin' || $act === 'load_session' || $act === 'buy') {
                ...
                if ($USE_SEAMLESS) {
                    ...
                    $userPlayer = BonusJokerPlayer::where('player_uuid', $userNameAgent)->first();
            ...
            if ($act === 'icons') {
                $session = BonusJokerPlayer::where('uuid', $token)->first();
                // var_dump(($gameRule));
                if ($session) {
                    if ($gameData) {
                                ...
                                if ($pull) {
                                    ...
                                    for ($i = 0; $i < count($resulJson); $i++) {
                                        ...
                                        for ($j = 0; $j < count($resulJson[$i]->win_drop); $j++) {
                                            ...
                                            $resulJson[$i]->win_drop[$j]->win_amount = $resulJson[$i]->win_drop[$j]->win_amount * $ajustRatio;
                                    ...
                                    $spinLogs = new BonusJokerSpinLogs;
                                    $data = [
                                    ...
                                    $lastid = BonusJokerSpinLogs::latest()->first()->uuid;
                                    ...
                                    if ($parentId != 0 && $freeModeCheck || $parentId != 0 && $freeMode) {
                                        $recordFree = BonusJokerSpinLogs::where('uuid', $parentId)->first();
                                        ...
                                        BonusJokerSpinLogs::where('uuid', $parentId)->update(['win_amount' => $winAmount, 'drop_feature' => $dropFeature]);
                                    }
            ...
            if ($act === 'histories') {
                ...
                if ($session) {
                    ...
                    $totalBet = (float) BonusJokerSpinLogs::where($search)->sum('total_bet');
                    $totalWin = (float) BonusJokerSpinLogs::where($search)->sum('win_amount');
                    $limit = 15;
                    // var_dump($totalWin);
                    $freeRequire = $gameData->freespin_require;
                    $paginate = BonusJokerSpinLogs::where($search)
            ...
            if ($act === 'history_detail') {
                ...
                if ($session) {
                    ...
                    $resultJson = [];
                    $history = BonusJokerSpinLogs::where("uuid", $uuid)->first()->toArray();
                    ...
                    if ($history) {
                        $historyChild = BonusJokerSpinLogs::where("parent_id", $uuid)->get()->toArray();
                        ...
                        if ($betamount && $bet_level) {
                            ...
                            if ($wallet >= $totalBet && $totalBet <= $MAX_BET && $totalBet >= $MIN_BET || $freeMode) {
                                ...
                                if ($pull) {
                                    ...
                                    // SlotgenBonusJoker::array_swap($reelOnScreenNewExplode, $number1 % 9, $number2 % 9);
                                    // SlotgenBonusJoker::array_swap($reelOnScreenNewExplode, $number3 % 9, $number4 % 9);
    ...
    {
        $playerId = BonusJokerPlayer::where('player_uuid', $playerId)->first();
