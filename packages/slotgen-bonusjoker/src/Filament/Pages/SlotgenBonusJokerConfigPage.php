...
namespace Slotgen\SlotgenBonusJoker\Filament\Pages;
...
use Slotgen\SlotgenBonusJoker\Models\SlotgenGame;
use Yepsua\Filament\Forms\Components\RangeSlider;
use ZipArchive;

class SlotgenBonusJokerConfigPage extends Page
{
    use InteractsWithForms;

    protected static ?string $navigationIcon = 'heroicon-o-document-text';

    protected static string $view = 'slotgen-bonusjoker::filament.pages.config-page';

    public ?array $data = [];

    public SlotgenBonusJokerConfig $setting;

    public function mount(): void
    {
        $rtpConfig = SlotgenBonusJokerConfig::first();
        if (!empty($rtpConfig)) {
    ...
    public function submit(): void
    {
        ...
        try {
            // dd($this->data);
            $setting = SlotgenBonusJokerConfig::first();
            if (!empty($setting)) {
                ...
                // dd($this->data);
                if (SlotgenBonusJokerConfig::create($this->data)) {
                    Notification::make()
                        ->title('Setting add')
    ...
    public function extractFile($fileName, $language = "en")
    {
        ...
        if ($folder != null) {
            ...
            // $data = [
            //     'game_name'        => $name,
            //     'game_class'     => "Slotgen\\SlotgenBonusJoker\\SlotgenBonusJoker",
            // ];
            // $slotgen->fill($data);
            // $slotgen->save();
            $gamePrivateFolder = storage_path('app/private/bonus_joker');
            $localtionStorage = 'app/private/bonus_joker';
            ...
            if (File::exists($gamePrivateFolder)) {
                File::deleteDirectory($gamePrivateFolder);
            }
            ...
            //File::copy(__DIR__ . '/../../../resources/games/bonusjoker-logo.png', $gamePublicFolder . '/bonusjoker-logo.png');
        } 
    }
}
